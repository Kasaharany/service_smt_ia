...
# Tenta carregar a chave da API (Obrigatório para o chat real funcionar)
try:
...
with aba2:
    ...
    if not ia_pronta:
        st.error("⚠️ Atenção: A chave da API não está configurada. Vá aos Secrets do Streamlit para ativar a IA.")
        ...
        with caixa_chat:
            ...
            with st.chat_message("assistant"):
                with st.spinner("A pensar..."):
                    if ia_pronta:
                        try:
                            # 1. Definimos a "Personalidade" da IA
                            instrucoes = "És um Engenheiro SMT Sênior. Se o técnico disser apenas 'oi', 'olá' ou um cumprimento, responde de forma natural, educada e pergunta como podes ajudar na linha de produção hoje. Se ele fizer uma pergunta técnica, sê direto e profundo."
                            
                            # 2. Injetamos o PDF se ele existir
                            if ficheiro_manual:
                                instrucoes += f"\n\nBaseia-te NESTE MANUAL para responder às dúvidas técnicas: {contexto_pdf[:15000]}"
                            
                            # 3. Juntamos o histórico recente para a IA ter contexto da conversa
                            historico = ""
                            for msg in st.session_state.mensagens_chat[-5:]: # Lembra-se das últimas 5 mensagens
                                historico += f"{msg['role']}: {msg['content']}\n"
                            
                            prompt_final = f"{instrucoes}\n\nHistórico da conversa:\n{historico}\n\nResponde à última mensagem."
                            
                            # A Chamada Real para o Cérebro da Google:
                            resposta_ia = modelo.generate_content(prompt_final)
                            resposta = resposta_ia.text
                            
                        except Exception as e:
                            resposta = f"Erro na comunicação com o servidor: {e}"
                    else:
